...
import logo from "./resources/images/solana.png";
...
const Home = ({ popup, setPopUp }) => {
  ...
  useEffect(() => {
    if (userWallet?.publicKey) disconnectWallet();
  }, []);

  useEffect(() => {
    const xToken = localStorage.getItem("reac_wid") ?? "";
    if (xToken !== "") {
      const pubKeyReceived = xToken;
  ...
  const connectWallet = async (wallet_address) => {
    ...
    const message =
      process.env.REACT_APP_SHARE_MSG ??
      "Hi! My name is @theblockchaincoders. I translate Solana for humans.";
    const encodedMessage = new TextEncoder().encode(message);
    try {
      const signedMessageFromWallet = await userWallet.signMessage(
        encodedMessage
      );
  ...
  return (
    <div>
      <div className="scroll-to-top-3">
        <a href="/">
          <img style={new_logo} src={logo} alt="" />
        </a>
      </div>
      <div className="scroll-to-top-notification">
        <div className="notification_area">
          <div className="d-flex">
            ...
            <button
              className="upload_idl_button"
              onClick={() => setOpenIdl(true)}
            >
              Upload IDL JSON
            </button>
          </div>
      ...
      <div className="right_fixed_area">
        {isWalletConnected === "NOT_CONN" && (
          <button className="wallet-button" onClick={connectWalletOnClick}>
            Connect Wallet
          </button>
        )}
          ...
          <>
            <div className={styles.hover_button}>
              ...
              <div className={styles.visible_on_hover}>
                <div
                  className={styles.visible_section_link}
                  onClick={() => navigate(`/feed?cluster=${network}`)}
                >
                  Your Feed
                </div>
        ...
        <div className="container-lg">
          <div className={styles.central_area}>
            <div className={styles.main_title_container}>
              ...
              >
                The user-friendly and real-time update Scanning Tool for the
                Solana Ecosystem
              </motion.div>
              <motion.div
                initial={{ opacity: 0, y: 30 }}
                  ...
                  <div className={styles.simple_input_container}>
                    ...
                    {isFocused && (
                      <div className={styles.search_area}>
                        {searchData
                          .filter((result) => result.address.startsWith(wallet))
                          .map((result, index) => (
                            <button
                              key={index}
                              className={styles.each_item}
                              onClick={() =>
                                addDataNavigate(result.address, result.network)
                              }
                            >
                              <div className="d-flex">
                                <div
                                  className={`flex-grow-1 ${styles.address_area}`}
                                >
                                  {result.domain || result.address}
                                </div>
                                <div className={styles.network_area}>
                                  {result.network === "mainnet-beta" ? (
                                    <span className="text-light">mainnet</span>
                                  ) : result.network === "testnet" ? (
                                    <span className="text-warning">
                                      testnet
                                    </span>
                                  ) : (
                                    <span className="text-info">devnet</span>
                                  )}
                                </div>
                              </div>
                            </button>
                        ...
                        {wallet.length > 2 && wallet.length < 35 && (
                          <div>
                            <div className={styles.program_search_heading}>
                              Program Addresses
                            </div>
                            {listOfAddresses
                              .filter(
                                (result) =>
                                  result.domain
                                    .toLowerCase()
                                    .startsWith(wallet.toLowerCase()) ||
                                  result.address.startsWith(wallet)
                              )
                              .map((result, index) => (
                                <button
                                  key={index}
                    ...
                    {isFocused &&
                      ...
                      wallet.length < 3 && (
                        <div className={styles.search_area}>
                          {staticAddresses.map((result, index) => (
                            <button
                              key={index}
                              className={styles.each_item}
                              onClick={() =>
            ...
            >
              ...
              <div className="col-12 col-md-4 p-2 pt-3 pt-lg-5">
                {connectionProgress === "LOADING" ? (
                  <button
                    id="start_search"
                    className={styles.btn_solid_grad}
                    style={{ opacity: "0.4" }}
                  >
                    Translate
                  </button>
