      ...
      <div className="modal-dialog">
        <div className={`modal-content ${styles.modal_body_outer}`}>
          <div className="modal-header border-bottom-0">
            <h4 className={`modal-title ${styles.modal_heading}`}>Welcome</h4>
            <button
              type="button"
              className="btn text-light"
              onClick={() => setShowFoll(false)}
            >
              ✖
            </button>
          </div>
                ...
                <div className={styles.each_follower} key={follow.id}>
                  <div className="d-flex justify-content-between">
                    <div className={styles.wallet_address_name}>
                      {shortenAddress(follow.followed_address) ?? "--"}
                    </div>
                    <div>
                      {remId === follow.id ? (
                        <div
                        ...
                        <button
                          className={styles.wallet_unfollow_button}
                          onClick={() =>
                            unfollowuser(
                              follow.id,
                              follow.followed_address,
                              follow.cluster
                            )
                          }
                        >
