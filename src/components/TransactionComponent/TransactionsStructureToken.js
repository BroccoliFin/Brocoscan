...
const TransactionStructureToken = ({ styles, id, data, address, cluster }) => {
  ...
  useEffect(() => {
    if (data.type === "SOL_TRANSFER") {
      data.actions.forEach((txn) => {
        if (data.type === txn.type) {
          if (address === txn.info.sender) setTxType("SOL Sent");
          else if (address === txn.info.receiver) setTxType("SOL Received");
        }
      });
    }
    if (data.type === "TOKEN_TRANSFER") {
      data.actions.forEach((txn) => {
        if (data.type === txn.type) {
          if (address === txn.info.sender) setTxType("Token Sent");
          else if (address === txn.info.receiver) setTxType("Token Received");
        }
      });
    }
    if (data.type === "NFT_TRANSFER") {
      data.actions.forEach((txn) => {
        if (data.type === txn.type) {
          if (address === txn.info.sender) setTxType("NFT Sent");
          else if (address === txn.info.receiver) setTxType("NFT Received");
        }
      });
    }
    if (data.type === "NFT_SALE") {
      data.actions.forEach((txn) => {
        if (data.type === txn.type) {
          if (address === txn.info.seller) setTxType("NFT Sold");
          else if (address === txn.info.buyer) setTxType("NFT Purchased");
        }
      });
    }
    if (data.type === "NFT_SALE") {
  ...
  return (
    <div>
      <motion.div
        ...
        viewport={{ once: true }}
        className={styles.each_txn_3}
      >
        <Link
          to={
            cluster === "mainnet-beta"
              ? `/tx/${data.signatures[0]}`
              : `/tx/${data.signatures[0]}?cluster=${cluster}`
          }
          style={{ textDecoration: "none" }}
          target="_blank"
        >
          <div className={styles.clickable_section}></div>
        </Link>
              ...
              <div>
                ...
                >
                  <Link
                    ...
                    target="_blank"
                  >
                    {shortenAddress(data.signatures[0])}
                  </Link>
                </Tooltip>
              </div>
            </div>
          ...
          >
            <div className={styles.txn_status} style={{ cursor: "pointer" }}>
              ...
              >
                ...
                {data.hasOwnProperty("status") && data.status === "Fail" ? (
                  <img src={failedTick} />
                ) : (
                  ""
                )}
          ...
          <div className="pe-2">
            ...
            >
              <motion.button
                ...
                onClick={() => copyValue(data.signatures[0])}
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
              >
          ...
          <div className="col-12">
            <div className={styles.fields_container}>
              <div className={styles.txn_name_section}>
                <div className={styles.responsive_spacer}></div>
                <div className="d-flex flex-wrap justify-content-start">
                    ...
                    <div className={styles.txn_name}>
                      {txType ||
                        (data.type === "UNKNOWN"
                          ...
                          : formatNames(data.type) || "Protocol Interaction")}
                    </div>
                  </div>
                  <div className="">
                    <div className={styles.txn_subname}>
                      ...
                      >
                        {data.protocol.name != "" ? (
                          <div>
                            <a href={`/address/${data.protocol.address}`}>
                  ...
                  <div className="">
                    <div
                      className={`${styles.txn_subname} ${styles.push_to_next}`}
                ...
                <div className={styles.responsive_spacer_bottom}></div>
              </div>
              ...
              {data.actions.length > 0
                ? data.actions.map((action, index) =>
                    isParsable(action.type) ? (
                      <SubTransactions
                        ...
                        key={index}
                        showRoyalty={true}
                        saleNftCreators={saleNftCreators}
                      />
                    ) : (
                      ""
                    )
                ...
                : "-"}
              {data.actions.length > 0 &&
                data.actions.length === unknownCount && (
                  <SubTxnUnknown styles={styles} unknownCount={unknownCount} />
                )}
