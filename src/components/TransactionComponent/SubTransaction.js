...
import collectFees from "../../resources/images/txnImages/collect_fees.png";
...
const SubTransactions = ({
    ...
    try {
      if (type === "TOKEN") {
        ...
        if (metadata_uri !== "") {
          ...
          if (nft_name !== "") setName(nft_name);
          const res = await getMetadata(metadata_uri);
          if (res.success === true) setImage(res.details.image ?? noImage);
        }
      } else if (type === "NONE") {
      } else {
  ...
  const getCurrency = async (cluster, address) => {
    try {
      if (address === "So11111111111111111111111111111111111111112") {
        setCurrency("SOL");
        if (preBalance !== 0) setPostBalance(formatLamports(preBalance) ?? 0);
        setDataLoaded(true);
      } else {
        const res = await getTokenData(cluster, address);
        if (res.success === true) {
      ...
      if (data.type === "SOL_TRANSFER") {
        ...
        type_obj = {
          ...
          loan_duration_seconds:
            convertToDays(data.info.loan_duration_seconds) ?? "--",
        };
        setRelField(data.info.nft_address ?? "");
        setCurrencyField(data.info.currency ?? "");
        ...
        type_obj = {
          ...
          grace_period_seconds:
            convertToDays(data.info.grace_period_seconds) ?? "--",
        };
        setRelField(data.info.nft_address ?? "");
        ...
        type_obj = {
          ...
          slippage_paid:
            formatNumbers(data.info.slippage_paid?.toFixed(7)) ?? "",
          token: "--",
          action: "--",
          value: "",
          symbol: "",
        ...
        if (data.info.raffle_token !== "") setRelField(data.info.raffle_token);
        else {
          setName(data.info.raffle_address ?? "--");
          setImage(Foxy);
        }
        ...
        type_obj = {
          ...
          value: "--",
          symbol: "",
          fees_taken: data.info.fees_taken ?? [],
        };
        setRelField(data.info.liquidity_pool_address);
        setImage(collectFees);
        setRelType("NONE");
      } else if (data.type === "COLLECT_REWARD") {
        type_obj = {
          type: "COLLECT_REWARD",
          from: data.info.liquidity_pool_address ?? "--",
          to: data.info.liquidity_provider_address ?? "--",
          token: "--",
          action: "--",
          value: data.info.reward.amount ?? "--",
          symbol: "",
        };
        setRelField(data.info.reward?.token_address ?? "");
        setName(data.info.reward?.name ?? "");
        setImage(data.info.reward?.image_uri ?? "");
        setCurrency(data.info.reward?.symbol ?? "");
        setRelType("NONE");
        ...
        const messg = `${data.info.isReply ? "Reply to " : ""} ${
          data.info.chatType
        }`;
        setImage(memo);
        setName(messg);
  ...
  useEffect(() => {
    if (
      showRoyalty === true &&
      Array.isArray(saleNftCreators) &&
      saleNftCreators.length > 0
    ) {
        ...
        <div className={styles.txn_details}>
          <div className={styles.subtxn_token}>
            <div className="d-flex custom_wrap_mob">
              {(() => {
                if (
                  data.type === "ADD_LIQUIDITY" ||
                  data.type === "REMOVE_LIQUIDITY"
                ) {
                  return (
                    <>
                  ...
                  return (
                    <>
                      {data.info.lender ? (
                        <a
                          href={`/address/${data.info.lender}?cluster=${cluster}`}
                        >
                          {shortenAddress(data.info.lender)}
                        </a>
                      ) : (
                        <a
                          href={`/address/${data.info.loan}?cluster=${cluster}`}
                        >
                  ...
                  try {
                    if (data.info.vote_type === "Approve")
                      return (
                        <>
                          <span className={styles.approve_color}>Approve</span>
                          &nbsp;Proposal
                        </>
                      );
                    else if (data.info.vote_type === "Deny")
                      return (
                        <>
                          <span className={styles.minus_color}>Deny</span>
                          &nbsp;Proposal
                        </>
                      );
                    else if (data.info.vote_type === "Abstain")
                  ...
                  data.type === "BUY_TICKETS" ||
                  data.type === "REVEAL_WINNERS" ||
                  data.type === "CLOSE_RAFFLE"
                ) {
                  return (
                    <>
                      <a href={`/address/${name}?cluster=${cluster}`}>
                        {shortenAddress(name)}
                      </a>
                    </>
                  );
                  ...
                  return (
                    <>
                      <a href={`/address/${varFields.to}?cluster=${cluster}`}>
                        {varFields.value}
                      </a>
                    </>
                  );
                  ...
                  return (
                    <>
                      Governance Config Set for{" "}
                      <a href={`/address/${relField}?cluster=${cluster}`}>
                        {shortenAddress(varFields.value)}
                      </a>
                    </>
                  );
                } else if (
                  ...
                  data.type === "COMPRESSED_NFT_SALE" ||
                  data.type === "COMPRESSED_NFT_LIST" ||
                  data.type === "COMPRESSED_NFT_LIST_CANCEL" ||
                  data.type === "COMPRESSED_NFT_TAKE_BID"
                ) {
                  return (
                    <>
                      {relField ? (
                        name === "" ? (
                          <a
                            href={`/address/${relField}?cluster=${cluster}&compressed=true`}
                          >
                            {shortenAddress(relField)}
                          </a>
                        ) : (
              ...
              {data.type === "CAST_VOTE" ||
              data.type === "CREATE_REALM" ||
              data.type === "CREATE_MINT_GOVERNANCE" ||
              data.type === "CREATE_TOKEN_GOVERNANCE" ? (
                <div className={styles.copy_bt}></div>
              ) : (data.type === "BUY_TICKETS" ||
                  data.type === "REVEAL_WINNERS" ||
                  data.type === "CLOSE_RAFFLE") &&
                name !== "" ? (
          ...
          {(() => {
            ...
            } else if (varFields.type === "COLLECT_REWARD") {
              return (
                <div>
                  <div className="row pt-1">
                    <div className="col-12 col-md-8">
                  ...
                  <div className="row pt-1">
                    <div className="col-12 col-md-6">
                      <div className="d-flex custom_wrap_mob">
                        ...
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                            {shortenAddress(varFields.tickets) ?? "--"}
                          </div>
                        </div>
                        <div>
                          <img
                    ...
                    <div className="col-12 col-md-6">
                      <div className={`text-end ${styles.field_sub_2}`}>
                        {varFields.tickets ?? "--"} x {varFields.value}{" "}
                        {currency}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="row pt-1">
                    <div className="col-12 col-md-8">
                      <div className="d-flex custom_wrap_mob">
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                            This Merkle Tree was created by{" "}
                          </div>
                        </div>
                        <div className="">
                          <div className={styles.field_sub_1}>
                  ...
                  {varFields.liquidity_details.length > 0 &&
                    varFields.liquidity_details.map((liquidity) => (
                      <div className="row pt-2">
                        <div className="col-12 col-md-10">
                          <div className="d-flex custom_wrap_mob">
                            <div className="pe-2">
                              <img
                                src={
                                  liquidity.symbol === "SOL"
                                    ? solanaIcon
                                    : liquidity.image_uri.includes(
                                        "ray-initiative.gift"
                                      ) ||
                                      liquidity.image_uri.includes(
                                        "dex-ray.gift"
                        ...
                        <div className="col-12 col-md-2 text-end"></div>
                      </div>
                    ))}
                </>
              );
            ...
            } else if (varFields.type === "COLLECT_FEES") {
              return (
                <>
                  {varFields.fees_taken.length > 0 &&
                    varFields.fees_taken.map((token) => (
                      <div className="row pt-2">
                        <div className="col-12 col-md-8">
                  ...
                  <div className="row pt-1">
                    <div className="col-12 col-md-12">
                      <div className="d-flex custom_wrap_mob">
                        ...
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                            created program governance in realm
                          </div>
                        </div>
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                  ...
                  <div className="row pt-1">
                    <div className="col-12 col-md-12">
                      <div className="d-flex custom_wrap_mob">
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                            Signatory removed from proposal
                          </div>
                        </div>
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                  ...
                  <div className="row pt-1">
                    <div className="col-12 col-md-12">
                      <div className="d-flex custom_wrap_mob">
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                            Transaction inserted to proposal
                          </div>
                        </div>
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                  ...
                  <div className="row pt-1">
                    <div className="col-12 col-md-12">
                      <div className="d-flex custom_wrap_mob">
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
                            Transaction removed from proposal
                          </div>
                        </div>
                        <div className="pe-1">
                          <div className={styles.field_sub_1}>
