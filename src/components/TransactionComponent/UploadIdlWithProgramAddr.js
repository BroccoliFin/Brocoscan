...
const UploadIdlWithProgramAddr = ({ setOpenIdl }) => {
  ...
  const submitIdl = () => {
    try {
      setMsg("");
      if (programAddr === "") setMsg("Program Address cannot be empty");
      else if (idlFile === null) setMsg("Please select a .json file");
      else {
  ...
  return (
    <div
      className="modal"
      tabIndex="-1"
      style={{ display: "block", backdropFilter: "blur(10px)" }}
    >
      <div className="modal-dialog modal-dialog-centered">
        <div
          className="modal-content"
          style={{
            backgroundColor: "#2a0855",
          ...
          <div className="modal-body upload_idl">
            ...
            <div className="idl_form_container">
              ...
              <div className="w-100 text-center pt-4 pb-1 mt-2">
                {status === "UNLOADED" && (
                  <button className="idl_upload" onClick={submitIdl}>
                    Upload
                  </button>
                )}
                {status === "LOADING" && (
                  <div
                    className="idl_upload"
                    style={{ maxWidth: "110px", margin: "0 auto" }}
                  >
