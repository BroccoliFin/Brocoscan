  ...
  await axios({
    ...
    .then(async (res) => {
      if (res.data.success === true) {
        ...
        try {
          if (
            Array.isArray(allCompressedNfts) &&
            allCompressedNfts.length > 0 &&
            allCompressedNfts.length < 100
          ) {
...
export async function categorizeAddresswithExplorer(
  network,
  address,
  isCompressed = false
) {
...
export async function getTxnUptoSignature(network, address, uptoSign) {
  ...
  try {
    ...
    while (txnReceivedComplete === false) {
      ...
      counter++;
      if (counter > 5 || txnReceivedComplete === true) break;
    }
