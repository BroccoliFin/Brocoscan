...
export async function userLogon(wallet_address) {
  ...
  const response = await database.from("user_details").select();
      ...
      await axios({
        ...
        .then((res) => {
          ...
          if (
            ...
            res.data.message === "Limit_reached"
          ) {
            response = {
              success: false,
...
export async function unFollowUserOld(
  wallet_address,
  followed_address,
  cluster
) {
  ...
  if (response.data.length > 0) {
    ...
    else callbackToBeModified = currentUserCallback.data[0].callback_mainnet;
